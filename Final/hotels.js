...
function createPromise() {
  ...
  console.log("Ajout des hotels de Relais et Chateaux au fichier ListeRelais.json");
}
  ...
  return new Promise(function(resolve) {
    if (scrapingRound === 1) {
      for (let i = 0; i < Math.trunc(LHotels.length / 2); i++) {
        ...
        console.log("Ajout de l'URL de l'hotel " + i);
      }
      resolve();
      scrapingRound++;
...
function fillHotelInfo(url, index) {
  return new Promise(function(resolve, reject) {
    request(url, function(err, res, html) {
      ...
      console.log("Ajout du code postal et du prix de l'hotel " + index);
      resolve(LHotels);
    });
  });
}
...
prom
  ...
  .then(() => {
    console.log("Ecriture terminée, la liste des hotels du site est dans le fichier ListeRelais.json")
  });
