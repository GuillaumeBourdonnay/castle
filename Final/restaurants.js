...
function createPromises() {
  for (let i = 1; i <= 37; i++) {
    ...
    console.log("Page " + i + " des restaurants étoilés Michelin ajoutée au fichier restaurants.json");
  }
}
  ...
  return new Promise(function(resolve) {
    if (scrapingRound === 1) {
      for (let i = 0; i < restaurantsList.length / 2; i++) {
        ...
        console.log("Ajout de l'URL du restaurant" + i);
      }
      resolve();
      scrapingRound++;
    }
...
function fillRestaurantInfo(url, index) {
  return new Promise(function(resolve, reject) {
    request(url, function(err, res, html) {
      ...
      console.log("Ajout des informations du restaurant " + index );
      resolve(restaurantsList);
    });
  });
}
...
function saveRestaurantsInJson() {
  return new Promise(function(resolve) {
    try {
      console.log("Ecriture du fichier JSON");
...
Promise.all(LPromises)
  ...
  .then(() => {
    console.log("Ecriture terminée, la liste des restaurants étoilés est dans le fichier RestaurantsEtoiles.json")
  });
